...
function Bank(props) {
  ...
  const verify = async (evt) => {
    ...
    setError("");
    fetch(
      `https://api.paystack.co/bank/resolve?account_number=${accountCode}&bank_code=${Code}`,
      {
        method: "GET", // or 'PUT'
        headers: {
      ...
      .then((data) => {
        ...
        if (data.status === false) {
          setError(
            "Could not resolve account name, check your account number or bank"
          );
        }
        setButton(true);
                  ...
                  <div class="col-12">
                    <div class="card dash-card dash-card__records dash-card__posts p-5">
                      ...
                      <div class="settings-userdetails-section">
                        ...
                        <form
                          class="verifybank-onboard-form mw-400 mx-auto"
                            ...
                            <div className="col-12 mb-2">
                              <div className="form-group">
                                <select
                                  name=""
                                  id=""
                                  className="form-control form-select style2-input"
                                >
                                  <option value={bankName}>{bank}</option>
                                  {Object.entries(banks).map((item) => {
                                    return (
                                      <option
                                        onChange={() => setBankCode(item[1])}
                                        value={item[1]}
                                      >
                                        {item[0]}
                                      </option>
